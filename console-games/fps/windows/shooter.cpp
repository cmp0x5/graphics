...
int main()
{
	...
	while(running)
	{
		...
		if (GetAsyncKeyState((unsigned short)'A') & 0x8000) // 0x41
		{
		...
		if (GetAsyncKeyState((unsigned short)'D') & 0x8000) // 0x44
		{
			fPlayerX += sinf(fPerpendicular) * 3.0f * fElapsedTime;
			fPlayerY += cosf(fPerpendicular) * 3.0f * fElapsedTime;
		...
		if (GetAsyncKeyState(VK_LEFT) & 0x8000) // 0x25
			fPlayerAngle -= 0.8f * fElapsedTime;

		if (GetAsyncKeyState(VK_RIGHT) & 0x8000) // 0x27
			fPlayerAngle += 0.8f * fElapsedTime;

		if (GetAsyncKeyState((unsigned short)'W') & 0x8000) // 0x57
		{
		...
		if (GetAsyncKeyState((unsigned short)'S') & 0x8000) // 0x53
		{
			fPlayerX -= sinf(fPlayerAngle) * 5.0f * fElapsedTime;
			fPlayerY -= cosf(fPlayerAngle) * 5.0f * fElapsedTime;
		...
		wprintf_s(screen, 40, L"X=%3.2f, Y=%3.2f, A=%3.2f, Perp=%3.2f, FPS=%3.2f ", fPlayerX, fPlayerY, fPlayerAngle, fPerpendicular, 1.0f / fElapsedTime);
